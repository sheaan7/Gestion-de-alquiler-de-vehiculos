...
export async function obtenerVehiculo(id: number): Promise<Vehiculo> {
  const respuesta = await fetch(`/api/vehiculos/${id}`, { cache: "no-store" });
  if (!respuesta.ok) {
    const msg = await respuesta.text();
    throw new Error(msg || "No se encontró el vehículo");
  }
  return respuesta.json();
}
...
export async function crearVehiculo(input: VehiculoInput): Promise<Vehiculo> {
  ...
  if (!respuesta.ok) {
    const msg = await respuesta.text();
    throw new Error(msg || "No se pudo crear el vehículo");
  }
  return respuesta.json();
}
...
export async function actualizarVehiculo(id: number, input: VehiculoInput): Promise<Vehiculo> {
  ...
  if (!respuesta.ok) {
    const msg = await respuesta.text();
    throw new Error(msg || "No se pudo actualizar el vehículo");
  }
  return respuesta.json();
}
...
export async function eliminarVehiculo(id: number): Promise<void> {
  ...
  if (!respuesta.ok) {
    const msg = await respuesta.text();
    throw new Error(msg || "No se pudo eliminar el vehículo");
  }
}
